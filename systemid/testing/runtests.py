...
def test(*paths, **kwargs):
    ...
    Note: paths are specified relative to the systemid root directory in a unix
    ...
    Run one file:
    >> import systemid
    >> systemid.test("systemid/core/tests/test_basic.py")

    Run all tests in systemid/functions/ and some particular file:
    >> import systemid
    >> systemid.test("systemid/core/tests/test_basic.py", "systemid/functions")
    """
    ...
    t = SystemidTests(r, kw, post_mortem)
    if len(paths) > 0:
        t.add_paths(paths)
    else:
        t.add_paths(["systemid"])
    return t.test()
...
def doctest(*paths, **kwargs):
    ...
    Run one file:
    >> import systemid
    >> systemid.doctest("systemid/core/tests/test_basic.py")

    Run all tests in systemid/functions/ and some particular file:
    >> import systemid
    >> systemid.doctest("systemid/core/tests/test_basic.py", "systemid/functions")
    """
    verbose = kwargs.get("verbose", False)
    blacklist = kwargs.get("blacklist", [])
    r = PyTestReporter(verbose)
    t = SystemidDocTests(r, blacklist=blacklist)
    if len(paths) > 0:
        t.add_paths(paths)
    else:
        t.add_paths(["systemid"])
...
class SystemidTests(object):

    def __init__(self, reporter, kw="", post_mortem=False):
        self._post_mortem = post_mortem
        self._kw = kw
        self._count = 0
        self._root_dir = self.get_systemid_dir()
    ...
    def get_systemid_dir(self):
        """
        Returns the root systemid directory.
        """
        this_file = os.path.abspath(__file__)
        systemid_dir = os.path.join(os.path.dirname(this_file), "..", "..")
        systemid_dir = os.path.normpath(systemid_dir)
        return systemid_dir
    ...
    def get_paths(self, dir="", level=15):
        ...
        Example:
        >> get_paths(2)
        ['systemid/test_*.py', 'systemid/*/test_*.py', 'systemid/*/*/test_*.py']
        >> get_paths(6)
        ['systemid/test_*.py', 'systemid/*/test_*.py', 'systemid/*/*/test_*.py',
        'systemid/*/*/*/test_*.py', 'systemid/*/*/*/*/test_*.py',
        'systemid/*/*/*/*/*/test_*.py', 'systemid/*/*/*/*/*/*/test_*.py']
        """
        wildcards = [dir]
...
class SystemidDocTests(object):
    ...
    def test_file(self, filename):
        def setup_pprint():
            from systemid import pprint_use_unicode
            # force pprint to be in ascii mode in doctests
            pprint_use_unicode(False)

            # hook our nice, hash-stable strprinter
            from systemid.interactive import init_printing
            from systemid.printing import sstrrepr
